...
@switch($row->getType())
    ...
    @case(\WezomCms\Core\Settings\Fields\AbstractField::TYPE_MULTI_INPUT)
    @include('cms-core::admin.partials.fields.multi-input')
    @break
    ...
    {!! Form::imageUploader($name, $row->getValueObj(), route($routeName . '.delete-settings-file', [$row->getStorageId(), $locale ?? null]), $row->getAttributes()) !!}
    @break

    @case(\WezomCms\Core\Settings\Fields\AbstractField::TYPE_FILE)
    {!! Form::settingsFileUploader($name, $row->getValueObj(), route($routeName . '.delete-settings-file', [$row->getStorageId(), $locale ?? null]), $row->getAttributes()) !!}
    @break
