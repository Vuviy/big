...
<div class="section section--bg-white section--off-t-md section--off-b-md">
    <div class="container">
        <div class="text _fz-xl _fw-bold _color-black _mb-lg">
            @lang('cms-media-blocks::site.Популярные гаджеты')
        </div>
                ...
                @if($loop->iteration <= 4)
                    <div class="_cell _cell--12 _sm:cell--6 _md:cell--4 _df:cell--3">
                        @include('cms-ui::partials.cards.discount-card', [
                            'obj' => $card,
                            'classes' => 'discount-card--type-1 discount-card--dark discount-card--stretch'
                        ])
                    </div>
                @else
